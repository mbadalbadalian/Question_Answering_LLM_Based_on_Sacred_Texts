import pandas as pd
from transformers import pipeline,BertTokenizer,BertModel,BertForQuestionAnswering,DistilBertForQuestionAnswering, DistilBertTokenizer
import torch
...
def LoadBibleDFAndText(ESV_Bible_DF_filepath):
    ESV_Bible_DF = pd.read_csv(ESV_Bible_DF_filepath)
    ESV_Bible_DF = ESV_Bible_DF[ESV_Bible_DF['b'] == 1]
...
def ProcessQuestion(question,model,tokenizer,bible_embeddings,ESV_Bible_DF,bible_books):
    ...
    similarities = CosineSimilarity(question_embedding, bible_embeddings)

    #Gets the most similar verse
    most_similar_verse_index = similarities.argmax().item()
    temp_data = ESV_Bible_DF.iloc[most_similar_verse_index][['b', 'c', 'v']]
    book_id,chapter,verse = temp_data['b'],temp_data['c'],temp_data['v']
...
def GetInfo():
    ...
    BERT_most_relevant_verse_model_fine_tuned_filepath = 'Models/Bible_BERT_most_relevant_verse_model_fine_tuned'
    ditilBERT_model_fine_tuned_filepath = 'Models/Bible_DistilBERT_model_fine_tuned'
    bible_embeddings_filepath = 'Additional_Data/bible_embeddings_original.pt'
    ...
    ESV_Bible_DF,ESV_Bible_text = LoadBibleDFAndText(ESV_Bible_DF_filepath)
    ...
    LoadBibleEmbeddings(bible_embeddings,'Additional_Data/bible_embeddings_original.pt')
    context = LoadContext(ESV_Bible_DF)
    return BERTmodel,BERTtokenizer,bible_embeddings,ESV_Bible_DF,bible_books,distilBERTmodel,distilBERTtokenizer,context

#GetAnswer Function
def GetAnswer(question,BERTmodel,BERTtokenizer,bible_embeddings,ESV_Bible_DF,bible_books,distilBERTmodel,distilBERTtokenizer,context):
    #Get the most relevant verse
    answerBERT = ProcessQuestion(question,BERTmodel,BERTtokenizer,bible_embeddings,ESV_Bible_DF,bible_books)
